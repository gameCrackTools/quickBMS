...
int CMD_Codepage_func(int cmd) {
    ...
    p = VAR(0);
    if(!p[0] || myisdigitstr(p)) {
        g_codepage = VAR32(0);
    } else {
        for(i = 0; g_charset_codepage[i].identifier >= 0; i++) {
            if(!stricmp(p, g_charset_codepage[i].name)) {
                g_codepage = g_charset_codepage[i].identifier;
                break;
            }
        }
    }
...
int CMD_Set_func(int cmd) {
    ...
    } else if(NUM(1) == TYPE_TO_UNICODE) {    // utf8 to utf16
    ...
    } else if(NUM(1) == TYPE_FULLBASENAME) {
        var   = VAR(2); // this is var2!!!
        var = re_strdup(NULL, var, NULL);
        p = mystrrchrs(var, PATH_DELIMITERS);
        if(!p) p = var;
...
//#endif
        ...
        tea_setup(tea_ctx, key, ivec ? 1 : 0, tmp1, tmp2, tmp3, tmp4, tmp5);
