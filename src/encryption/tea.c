...
void tea_setup(tea_context *ctx, unsigned char key[16], int custom, uint32_t delta, uint32_t sum, uint32_t endian, uint32_t cycles, int inverted_sign) {
    int i;
    ...
    ctx->sum    = tea_setup ? 0 : 0xc6ef3720;
    ctx->endian = 1;
    ctx->cycles = 32;
    if(custom) {
        ctx->delta  = delta;
