...
#define append_mode_extracted_file(X) \
    ...
    if(!fname || !fname[0]) {
        if(g_input_total_files <= 1) {    // extension added by sign_ext
            ...
            spr(&tmp_fname, "%s%c%"PRIx".dat", g_filenumber[0].basename, PATHSLASH, g_extracted_files);
        }
        fname = tmp_fname;
        nametmp = 1;
...
int fputss(int fdnum, u8 *data, int chr, int unicode, int line, int maxsz) {  // writes a chr terminated string, currently unicode is referred to utf16
    ...
    for(i = 0;;) {
        ...
        if(unicode) {
            i += utf8_to_utf16_chr(data + i, maxsz - i, &wc, 0, g_codepage);
            c = wc;
        } else {
            c = data[i++];
        }
...
u8 *myfrx(int fdnum, int type, int *ret_num, int *error) {
    ...
    *error = 0;
    switch(type) {
        case TYPE_LONGLONG:     retn = fgetxx(fdnum, 8, error);     break;
        case TYPE_LONG:         retn = fgetxx(fdnum, 4, error);     break;
        case TYPE_SHORT:        retn = fgetxx(fdnum, 2, error);     break;
        case TYPE_BYTE:         retn = fgetxx(fdnum, 1, error);     break;
        case TYPE_THREEBYTE:    retn = fgetxx(fdnum, 3, error);     break;
        ...
        case TYPE_SIGNED_BYTE:         retn = fgetxx(fdnum, 1, error); if(retn & 0x80)       retn |= -0x100LL;         break;
        case TYPE_SIGNED_SHORT:        retn = fgetxx(fdnum, 2, error); if(retn & 0x8000)     retn |= -0x10000LL;       break;
        case TYPE_SIGNED_THREEBYTE:    retn = fgetxx(fdnum, 3, error); if(retn & 0x800000)   retn |= -0x1000000LL;     break;
        case TYPE_SIGNED_LONG:         retn = fgetxx(fdnum, 4, error); if(retn & 0x80000000) retn |= -0x100000000LL;   break;
        default: {
...
int myfwx(int fdnum, int varn, int type) {
    ...
    switch(type) {
        ...
        case TYPE_VARIABLE5:    retn = put_type_variable5(fdnum, get_var32(varn));  break;
