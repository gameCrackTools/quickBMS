...
#ifdef WIN32
    return _wchdir(native_utf8_to_unicode(folder));
#else
    return chdir(folder);
#endif
}
...
FILE *xfopen(u8 *fname, u8 *mode) {
    ...
    fd = fopen(fname, mode);
#ifdef WIN32
    if(!fd && (strlen(fname) > MAX_PATH)) {
        fname = long_name_support(fname);
        fd = fopen(fname, mode);
    }
    if(!fd) {
        wchar_t wmode[strlen(mode) + 20];
        swprintf(wmode, L"%s, ccs=UTF-8", native_utf8_to_unicode(mode));
        fd = _wfopen(native_utf8_to_unicode(fname), wmode);
    }
#endif
    if(fd) {
...
u8 *set_utf8_to_unicode(u8 *input, int input_size, int *ret_size) {
    ...
    i = 0;
    while(p < l) {
        p += utf8_to_utf16_chr(p, l - p, &wc, 1, g_codepage);
        if(!wc) break;
        if((i + 2) >= buffsz) {
