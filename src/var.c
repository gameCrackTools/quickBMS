...
int add_datatype(u8 *str) {
    if(str) {
        ...
        if(!stricmp(str, "FilePath"))       return(TYPE_FILEPATH);
        if(!stricmp(str, "FullPath"))       return(TYPE_FILEPATH);
        ...
        if(!stricmp(str, "TO_Unicode"))     return(TYPE_TO_UNICODE);
        if(!stricmp(str, "TO_UTF-16"))      return(TYPE_TO_UNICODE);
        if(!stricmp(str, "TO_UTF16"))       return(TYPE_TO_UNICODE);
        ...
        if(!stricmp(str, "variable5"))      return(TYPE_VARIABLE5);
        ...
        if(!stricmp(str, "fullbasename"))   return TYPE_FULLBASENAME;
        if(!stricmp(str, "none"))           return TYPE_NONE;
        if(!stricmp(str, "null"))           return TYPE_NONE;
